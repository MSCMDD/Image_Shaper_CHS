    ...
    public partial class Form_FireFLHFinder : Form
    {
        ...
        public CPalette Palette
        {
            ...
            set
            {
                _Palette = value;
                if (_Palette != null)
                    this.button_LoadPalette.Text = "调色板: " + _Palette.PaletteName;
                UpdateImages();
            }
        }
        ...
        public Form_FireFLHFinder()
        {
            ...
            toolTip1.SetToolTip(this.checkBox_IsLaser, "使用激光时，游戏（TS）会将其向右渲染1个像素。\n在使用激光时，检查此项以获得正确的火焰偏移。\n这对FireFLH不起作用。为此，你必须将整个单元1像素向右移到画布中。");
        }
        ...
        void uC_ImageCanvas_Left_PixelClicked(object sender, LocationEventArgs e)
        {
            ...
            if (this.radioButton_FirePixelOffset.Checked)
            {
                ...
                this.richTextBox1.SelectedText = "开火位置偏移=" + e.Location.X.ToString() + "," + e.Location.Y.ToString() + "\n";
                this.uC_ImageCanvas_Left.PreviewPixel = new Point[1] { e.Location };

            }
            if (this.radioButton_DamageFireOffset.Checked)
            {
                this.richTextBox1.SelectedText = "受损时开火位置偏移" + count + "=" + e.Location.X.ToString() + "," + e.Location.Y.ToString() + "\n";
                Points.Add(e.Location);
                this.uC_ImageCanvas_Left.PreviewPixel = Points.ToArray();
            }
            if (this.radioButton_MuzzleFlash.Checked)
            {
                this.richTextBox1.SelectedText = "枪口火焰" + count + "=" + e.Location.X.ToString() + "," + e.Location.Y.ToString() + "\n";
                Points.Add(e.Location);
                this.uC_ImageCanvas_Left.PreviewPixel = Points.ToArray();
            }
            ...
            if (this.radioButton_FireFLH.Checked)
            {
                ...
                this.richTextBox1.Clear();
                this.richTextBox1.SelectedText = "抛射体=" + FireFLH.ToString();
        ...
        void radioButton_CheckedChanged(object sender, EventArgs e)
        {
            ...
            this.groupBox_Left.Text = this.radioButton_FireFLH.Checked ? "面向西方的单位" : "位置偏移";
            this.groupBox_Right.Text = "面向北方的单位";
            if (this.radioButton_FireFLH.Checked)
        ...
        private void button_LoadSHP_Click(object sender, EventArgs e)
        {
            ...
            ofd.FileName = "";
            ofd.Filter = "SHP(TS) 图像文件|*.shp";
            ofd.Multiselect = true;
            if (ofd.ShowDialog() == DialogResult.OK)
            {
                if (!CSHaPer.IsSHP(ofd.FileName))
                {
                    MessageBox.Show("没有有效的SHP文件！");
                    return;
                }
