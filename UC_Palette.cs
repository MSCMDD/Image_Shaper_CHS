...
namespace ImageShaper
{
    public partial class UC_Palette : UserControl
    {
        ...
        public UC_Palette()
        {
            ...
            toolTip1.SetToolTip(this.comboBox_Palette, "不同调色板设置的列表。");
            toolTip1.SetToolTip(this.button_NewCopy, "创建新的空白调色板设置。");
            toolTip1.SetToolTip(this.button_Clone, "创建当前调色板设置的副本，不包括忽略/透明颜色的设置和所选颜色转换方法。");
        }
        ...
        private void LoadPalette()
        {
            ...
            if (AllowImageShaperPaletteSetup)
                ofd.Filter = "调色板文件|*.pal|ImageShaper 调色板设置|*.isps";
            else
                ofd.Filter = "调色板文件|*.pal";
        ...
        private void SavePalette()
        {
            ...
            sfd.Title = "保存调色板文件/设置";
            if ((_Palette == null) || (_Palette.PaletteFile == null)) return;
            sfd.FileName = System.IO.Path.GetFileName(_Palette.PaletteFile);

            if (AllowImageShaperPaletteSetup)
                sfd.Filter = "TS/RA2 调色板|*.pal|JASC 8bit 调色板|*.pal|ImageShaper 调色板设置|*.isps";
            else
                sfd.Filter = "TS/RA2 调色板|*.pal|JASC 8bit 调色板|*.pal";
            ...
            if (e.Button == MouseButtons.Right)
            {
                 ...
                 && (contextMenuSaveEnabled || contextMenuLoadEnabled))
                {
                    ...
                    if (currentpalette != null)
                    {
                        ...
                        if ((paletteinfo == null)||(paletteinfo == "")) paletteinfo = "临时调色板";

                        MenuItem info = new MenuItem(paletteinfo);
                        info.Enabled = false;
                        contextMenu.MenuItems.Add(info);

                        MenuItem UnloadPalette = new MenuItem("卸载调色板", new System.EventHandler(this.UnloadPaletteMenuItem_Click));
                        contextMenu.MenuItems.Add(UnloadPalette);

                        contextMenu.MenuItems.Add("-");

                        MenuItem MakeTransparent = new MenuItem("使透明", new System.EventHandler(this.MakeTransparentMenuItem_Click));
                        contextMenu.MenuItems.Add(MakeTransparent);

                        MenuItem IgnoreColor = new MenuItem("忽略颜色", new System.EventHandler(this.IgnoreColorMenuItem_Click));
                    ...
                    {
                        MenuItem info = new MenuItem("未加载调色板");
                        info.Enabled = false;
                        contextMenu.MenuItems.Add(info);
                    }
                    ...
                    {
                        MenuItem loadPalette = new MenuItem("加载调色板", new System.EventHandler(this.loadPaletteMenuItem_Click));
                        contextMenu.MenuItems.Add(loadPalette);
                    }
                    if (contextMenuSaveEnabled)
                    {
                        MenuItem savePalette = new MenuItem("保存调色板", new System.EventHandler(this.savePaletteMenuItem_Click));
